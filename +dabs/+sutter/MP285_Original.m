classdef MP285 < dabs.interfaces.LSCSerial
    ...
    methods
        ...
        function v = get.positionAbsoluteRaw(obj)
            try %RYOHEI %For some reason this decreases the crush.
                obj.hRS232.sendCommand('c');
                obj.hRS232.flushInputBuffer(); %Ryohei Should not be necessary.
                posn = obj.hRS232.readBinaryRaw(3,'int32');
                obj.hRS232.readBinaryRaw(1); %Ryohei
                v = posn(:)';
            catch ME %RYOHEI
                display('**Report to Ryohei: ERROR +dab\+sutter\MP285.m Ln 134');          
            end %RYOHEI           
        end
